...
class Puppet::Provider::CobblerSystemInterface::CobblerSystemInterface < Puppet::ResourceApi::SimpleProvider
  ...
  def get(context)
    ...
    unless @system_interfaces.length > 0 
      ...
      #simple_xlate = %w( name hostname owners profile image status kernel_options kernel_options_post autoinstall_meta boot_loader proxy netboot_enabled kickstart comment enable_gpxe server next_server filename gateway name_servers name_servers_search ipv6_default_device ipv6_autoconfiguration )
